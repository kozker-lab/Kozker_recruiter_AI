-- Migration: 20260610000000_init_schema
-- Description: Initialize full database schema for Kozker Recruiter AI ATS.
...
-- Create job_opening_skills table
...
-- Active views for Soft Deletes filter
CREATE OR REPLACE VIEW public.active_clients AS SELECT * FROM public.clients WHERE is_deleted = FALSE;
CREATE OR REPLACE VIEW public.active_requirements AS SELECT * FROM public.requirements WHERE is_deleted = FALSE;
CREATE OR REPLACE VIEW public.active_job_openings AS SELECT * FROM public.job_openings WHERE is_deleted = FALSE;
CREATE OR REPLACE VIEW public.active_candidates AS SELECT * FROM public.candidates WHERE is_deleted = FALSE;
CREATE OR REPLACE VIEW public.active_applications AS SELECT * FROM public.applications WHERE is_deleted = FALSE;

-- Trigger functions & utilities
...
RETURNS BOOLEAN AS $$
    SELECT COALESCE(EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin'), FALSE);
$$ LANGUAGE sql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION public.is_recruiter()
RETURNS BOOLEAN AS $$
    SELECT COALESCE(EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'recruiter'), FALSE);
$$ LANGUAGE sql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION public.is_manager()
RETURNS BOOLEAN AS $$
    SELECT COALESCE(EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'manager'), FALSE);
$$ LANGUAGE sql SECURITY DEFINER;
...
BEGIN
    ...
    VALUES (NEW.id, NEW.email, COALESCE(NEW.raw_user_meta_data->>'full_name', ''), 'recruiter', TRUE, FALSE)
...
-- Overwrite validate_job_skills_weight function to implement check correctly
CREATE OR REPLACE FUNCTION public.validate_job_skills_weight()
RETURNS TRIGGER AS $$
DECLARE
    v_total_weight NUMERIC;
...
CREATE TRIGGER trigger_candidate_deduplication BEFORE INSERT ON public.candidates FOR EACH ROW EXECUTE FUNCTION public.handle_candidate_deduplication();
CREATE TRIGGER trigger_interview_stage_failure_check BEFORE INSERT OR UPDATE ON public.interview_stages FOR EACH ROW EXECUTE FUNCTION public.check_stage_notes_on_failure();
CREATE TRIGGER trigger_job_skills_weight_check BEFORE UPDATE ON public.job_openings FOR EACH ROW EXECUTE FUNCTION public.validate_job_skills_weight();
...
VALUES ('resumes', 'resumes', false), ('candidate_documents', 'candidate_documents', false), ('avatars', 'avatars', true)
