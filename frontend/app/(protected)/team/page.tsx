...
export default function TeamOperationsPage() {
  ...
  return (
    <div className="space-y-6 pb-12 font-sans text-neutral-200">
      {/* Top Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-neutral-900 border border-neutral-800 p-6 rounded-sm shadow-sm">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-xl font-tight font-bold text-neutral-white tracking-tight">Team Operations & Recruiter Workload</h1>
        ...
        <div className="flex items-center gap-3 font-mono text-xs text-neutral-400">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-neutral-950 border border-neutral-800 rounded-sm">
            <ShieldCheck className="w-4 h-4 text-emerald-400" />
            <span>Supervision Scope:</span>
            {!isGlobalAdmin && (
              <span className="font-bold text-neutral-white px-2 py-0.5 bg-primary/20 text-primary border border-primary/30 rounded-xs text-[10px]">
          ...
          {isGlobalAdmin && (
            <select
              ...
              className="p-1.5 bg-neutral-950 border border-neutral-800 text-neutral-200 rounded-sm font-mono text-xs focus:outline-none focus:border-primary"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 bg-neutral-900 border border-neutral-800 rounded-sm space-y-1">
          <div className="flex items-center justify-between text-neutral-400 text-xs font-mono">
            <span>Reporting Recruiters</span>
            <Users className="w-4 h-4 text-primary" />
          </div>
          <div className="text-2xl font-bold text-neutral-white font-mono">{summary.total_recruiters}</div>
        ...
        <div className="p-4 bg-neutral-900 border border-neutral-800 rounded-sm space-y-1">
          ...
          <div className="text-2xl font-bold text-neutral-white font-mono">{summary.active_jobs}</div>
        ...
        <div className="p-4 bg-neutral-900 border border-neutral-800 rounded-sm space-y-1">
          ...
          <div className="text-2xl font-bold text-neutral-white font-mono">{summary.pending_reviews}</div>
        ...
        <div className="p-4 bg-neutral-900 border border-neutral-800 rounded-sm space-y-1">
          ...
          <div className="text-2xl font-bold text-neutral-white font-mono">{summary.delayed_actions}</div>
          <p className="text-[10px] text-neutral-500 font-mono">Exceeding SLA turnaround</p>
        </div>
      </div>

      {/* Recruiter Workload Breakdown Table */}
      <div className="bg-neutral-900 border border-neutral-800 rounded-sm overflow-hidden shadow-sm">
        <div className="p-4 border-b border-neutral-800 flex items-center justify-between">
          <h3 className="font-bold text-neutral-white text-sm">Recruiter Workload & Performance Table</h3>
          <span className="font-mono text-[10px] text-neutral-400 uppercase tracking-wider">Live Supervision Data</span>
        </div>
                    ...
                    <tr key={member.id} className="hover:bg-neutral-800/40 transition-colors">
                      ...
                      <td className="p-3.5 font-mono text-[11px]">
                        <div className="flex flex-wrap gap-1.5">
                          {roles.length > 0 ? (
                            roles.map((r: any) => (
                              <span key={r.id} className="px-2 py-0.5 rounded bg-primary/10 text-primary border border-primary/20 text-[10px] font-semibold">
                                {r.name} [{r.branch_name || 'Main Branch'}]
                              </span>
                            ))
                          ) : (
                            <span className="px-2 py-0.5 rounded bg-neutral-800 text-neutral-400 border border-neutral-700 text-[10px]">
                              Recruiter
                            </span>
                          )}
                        </div>
                      ...
                      <td className="p-3.5 text-right">
                        <button
                          ...
                          className="px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 font-mono text-[11px] font-bold rounded-sm inline-flex items-center gap-1.5 transition-colors cursor-pointer border border-neutral-700"
                        >
      ...
      {selectedRecruiter && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-neutral-900 border border-neutral-800 max-w-lg w-full p-6 rounded-sm shadow-2xl space-y-4 text-neutral-200 text-xs">
            <div className="flex items-center justify-between border-b border-neutral-800 pb-3">
              <div className="flex items-center gap-2.5">
                <UserCheck className="w-5 h-5 text-primary" />
                <div>
                  <h3 className="font-bold text-neutral-white text-sm">Recruiter Supervision Summary: {selectedRecruiter.name}</h3>
            ...
            <div className="grid grid-cols-2 gap-2 font-mono text-[11px] p-3 bg-neutral-950 border border-neutral-800 rounded-sm">
              <div>
            ...
            <form onSubmit={handleSendMessage} className="space-y-3 pt-2">
              <label className="block font-mono text-[10px] font-bold uppercase text-neutral-400 tracking-wider">
              ...
              <textarea
                ...
                required
                rows={3}
                className="w-full p-2.5 bg-neutral-950 border border-neutral-800 text-neutral-200 rounded-sm text-xs focus:outline-none focus:border-primary font-sans placeholder:text-neutral-600"
              />

              {messageSentMsg && (
                <div className="p-2.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono text-[11px] rounded-sm flex items-center justify-center gap-2">
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-neutral-800">
                <button
                  ...
                  className="px-3.5 py-2 border border-neutral-800 hover:bg-neutral-800 rounded-sm text-xs font-semibold cursor-pointer"
                >
                  Close
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-primary hover:bg-primary/90 disabled:opacity-50 text-neutral-white text-xs font-mono font-bold uppercase tracking-wider rounded-sm cursor-pointer transition-colors"
                >
